...
def test_real_market_paper_runner_persists_runtime_events_to_database(tmp_path):
    import json
    ...
    with Session(engine) as session:
        events = session.execute(select(PaperRuntimeEvent).order_by(PaperRuntimeEvent.id)).scalars().all()

    assert [event.event_type for event in events] == ["signal", "snapshot", "signal", "snapshot", "fill"]
    assert events[0].strategy_type == "TREND_PULLBACK"
    assert events[0].action == "LONG_ENTRY"
    assert events[-1].action == "EXIT"
    assert Decimal(json.loads(events[-1].payload)["net_pnl"]) == Decimal("100.000")


def test_real_market_paper_runner_persists_rejected_signal_events(tmp_path):
    from sqlalchemy import create_engine, select
    from sqlalchemy.orm import Session, sessionmaker
    ...
    asyncio.run(
        run_real_market_paper(
            RealMarketPaperConfig(
                ...
                slippage_pct=Decimal("0"),
                max_fee_to_risk_ratio=Decimal("0.01"),
                event_session_factory=session_factory,
            ),
            source=source(),
            signal_fn=signal_fn,
    ...
    assert [event.event_type for event in events] == ["signal", "snapshot", "rejected_signal"]
    assert events[-1].action == "LONG_ENTRY"
    assert '"rejected_signal": true' in events[-1].payload
