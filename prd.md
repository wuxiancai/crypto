...
版本：v0.3-dev-ready  
日期：2026-06-16  
状态：开发前修订版，先用于架构、任务拆分和回测/Paper Trading 原型开发  

## 1. 项目定位

本项目是一个面向币安 USDⓈ-M Futures 的自动交易研发系统，核心策略为：
...
1. 风控优先于信号。
2. 数据质量优先于策略判断。
3. 所有实盘订单必须有可恢复的订单状态机。
4. 任何不确定状态默认禁止新开仓。
5. AI 只能输出 `ALLOW`、`WARN`、`BLOCK`，不能下单、不能改参数、不能覆盖硬风控。
6. 回测、Paper Trading、Live Trading 必须共享同一套策略与风控规则。
...
实时接收币安行情，但不发送真实订单。Paper 模式必须使用与 Live 相同的风控、订单计划和持仓状态机，只替换执行适配器。

### 4.3 Live Trading

真实连接币安 API。Live 模式默认关闭，必须显式配置开启，并通过启动前自检。
...
MVP 目标是先实现“数据 -> 指标 -> 信号 -> 风控 -> Paper/回测 -> 可复盘”的闭环，不急于实盘。
...
- EMA50、EMA200、ADX、ATR、Bollinger Bands
- 趋势识别、波动率识别、回踩做多、反弹做空
- 趋势转换试仓信号识别、回测统计与 Paper 验证
- 单笔风险仓位计算
...
- `strategy`：根据已收盘 K 线和指标生成候选信号，必须包含 `pullback_strategy`、`reversal_strategy`、`reversal_detector`、`reversal_score`。
...
def get_closed_multi_timeframe_context(current_time, symbol):
    ...
    return {"15m": kline_15m, "1h": kline_1h, "4h": kline_4h}
```
...
4h 与 1h 冲突时，状态为 `TRANSITION`。`TRANSITION` 不等于全系统 `WAIT`：
...
- 4h 看空、1h 看多：`main_strategy_action = WAIT`，`reversal_strategy_action = EVALUATE_REVERSAL_LONG`
- 4h 看多、1h 看空：`main_strategy_action = WAIT`，`reversal_strategy_action = EVALUATE_REVERSAL_SHORT`
- 4h 与 1h 都看多：主策略评估多头，趋势转换禁用
- 4h 与 1h 都看空：主策略评估空头，趋势转换禁用
- EMA50 / EMA200 缠绕且 1h / 15m 无明确结构：主策略与趋势转换都等待
...
```json
{
  ...
  "reason": ["4h remains weak", "1h close > EMA200", "15m may be turning bullish"]
}
```
...
- 4h trend = `UPTREND`
- 1h trend = `UPTREND` 或 `BULLISH_CONFIRM`
- ADX >= `min_adx`
- DI_PLUS > DI_MINUS
- ATR_PCT 在允许范围内
...
- 15m low <= EMA50 * (1 + `pullback_tolerance_pct`)
- 15m close 接近 EMA50
...
- 4h trend = `DOWNTREND`
- 1h trend = `DOWNTREND` 或 `BEARISH_CONFIRM`
- ADX >= `min_adx`
- DI_MINUS > DI_PLUS
- ATR_PCT 在允许范围内
- AI/Funding/账户风控未 BLOCK
...
- 15m high >= EMA50 * (1 - `rebound_tolerance_pct`)
- 15m close 接近 EMA50
...
```json
{
  ...
  "reason": ["4h uptrend", "1h trend confirmed", "15m pullback to EMA50"],
...
HEDGE 模式不作为 MVP 默认实现。未来启用 HEDGE 必须单独配置 `positionSide`、`reduceOnly`、`closePosition` 的行为。
...
趋势转换试仓不是边缘扩展，而是本系统的核心策略之一。它用于捕捉 4h 大周期尚未完全反转，但 1h 和 15m 已经率先完成结构切换的行情。
...
趋势转换试仓必须纳入 MVP 的信号识别、回测统计和 Paper Trading 验证；但在没有通过专项验收前，不允许进入 Live 实盘。
...
- 能完成无未来函数回测。
- 能完成 Paper Trading。
- 能在回测和 Paper 中生成并统计趋势转换早期/确认试仓信号。
...
- 没有回测，不能进入 Paper。
- 没有 Paper，不能进入 Live。
- 没有风控和 Kill Switch，不能自动下单。

## 26. 当前必须冻结的开发决策

1. MVP 默认不实盘，只做 Backtest + Paper。
2. AI 默认关闭，只保留接口和 deterministic stub。
3. 趋势转换试仓是核心策略，必须进入 MVP 的回测与 Paper 验证，但 Live 实盘默认关闭。
4. 所有交易计算使用 Decimal。
5. 风控、订单、持仓必须建状态机。
