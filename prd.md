...
Live 真实下单仍默认关闭。未来 Live 若要支持同一 symbol 多空共存，必须单独启用 Binance Futures `HEDGE` position mode，并通过独立自检、小资金配置和用户确认。
...
历史 `REVERSAL_PROBE` 不再作为新增 MVP 默认入口；分层策略系统中的 hedge 策略必须纳入回测统计和 Paper Trading 验证，但在没有通过专项验收前，不允许进入 Live 实盘。
...
历史 `REVERSAL_PROBE` 进入 Live 的最低要求：
...
- 交易对规则已同步
- position_mode = ONE_WAY
- Paper/Backtest 下一阶段使用 `STRATEGY_BUCKETS` 管理子仓；Live 默认仍为 `ONE_WAY`。
- margin_type = ISOLATED
...
3. 分层策略系统是下一阶段策略主线，必须进入 MVP 的回测与 Paper 验证，但 Live 实盘默认关闭。
4. 所有交易计算使用 Decimal。
5. 风控、订单、持仓必须建状态机。
...
1. 新增独立策略系统模块与参数配置。
2. 为六类策略写测试 fixture。
3. 将 Paper/Backtest 从单仓位升级为策略子仓。
