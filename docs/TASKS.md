...
- 当前 Web 状态页已新增“策略回测”按钮，点击后以新标签页打开 `/backtest`。回测页复用当前实时策略适配器和 PaperTradingEngine，用 Binance REST 历史 K 线回放 4h / 1h / 15m 多周期策略；默认 EMA50 / EMA200、历史 K 线 1500 根、1000 USDT，用户可输入 EMA 快线、EMA 慢线和历史 K 线根数比较不同参数。
...
- 待做：策略回测继续升级为分页历史回测，支持选择最近 3 个月 / 6 个月 / 1 年，自动分页拉取 Binance 历史 K 线，并输出最大回撤、胜率、盈亏比、按交易对统计和参数组合对比表。
