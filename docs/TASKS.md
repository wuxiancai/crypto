...
- 当前已实现真实行情 Paper runner 与脚本入口：`scripts/run_paper_realtime.py` 会连接 Binance WebSocket 已收盘 K 线流，并使用持久化 Paper stream runner 保存状态。当前默认策略函数为安全 `WAIT`，后续需要接入多周期策略编排后才会产生模拟交易。
