...
- [x] `/backtest/batch` 新增“停止回测”按钮和终端风格运行日志；页面轮询 `/api/backtest/batch/status` 展示 `[run]`、`[ARCHIVED]`、耗时、剩余预计和停止状态。停止请求会在当前组合结束后的安全点退出。
...
- 当前批量回测 Web 入口使用后台任务执行，页面提供停止按钮和运行日志面板；日志内容由脚本显式回调输出，接近 Ubuntu 终端里的批量回测进度展示。
