...
- 当前 Web 状态页已增加精简版“策略触发条件”：只展示当前最接近触发的策略方向，例如主趋势做空时不再显示主趋势做多；页面顶部显示交易对、当前趋势、已满足进度和“还差”条件，计算公式默认折叠到“计算明细”。主趋势诊断已拆分为空头/多头结构与动能确认，避免把肉眼可见的 EMA 空头结构误写成“下跌趋势未满足”。
