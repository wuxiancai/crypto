...
- [x] 2026-07-01 修正周线空仓初始防线来源：`WEEKLY` 入场默认使用周线 MA60 作为趋势防线，不再把 20 周结构高点当默认初始止损。
- [x] 2026-07-01 周线仓杠杆/保证金独立化：Paper/Backtest 开仓按 `position_level` 解析独立杠杆，默认 `WEEKLY=2X`、`DAILY=5X`、`H4=10X`；周线仓的强平保护、名义价值上限和持仓展示不再沿用全局 `10X`。
- [x] 2026-07-01 周线仓执行周期隔离：`WEEKLY` 开仓、减仓和退出只能在 `1w` 周线收盘事件执行；日线和 4H 事件即使重新计算出周线管理信号，也会被适配层降级为 `WAIT`，避免周线仓被 4H 回放节奏反复减仓。
- [x] 2026-07-01 周线 Lifecycle 阶段去重：周线趋势破坏、结构破坏、动能破坏会写入持仓 `lifecycle_state`，同一破坏阶段在同一周线仓内最多减仓一次；Paper/Backtest 上下文会回传该状态，避免连续多周重复执行同类减仓。
- [x] 2026-07-01 策略回测页和批量回测页统一新增“清除回测结果”入口；清除动作删除策略回测归档、交易记录和孤立配置快照，且不依赖批量回测运行开关。
- [x] 2026-07-01 状态页错误日志显示 UTC+8 时间：实时 Paper runner 后续写入的 stdout/stderr 日志行会自带时间戳；状态页读取旧的无时间戳错误行时按日志文件修改时间补时间，避免只显示错误内容看不到发生时间。
- [x] 2026-07-01 已新增独立时间线交易政策文档 `trade_policy.md`：明确周线、日线、4H 三种时间线单独立决策；不同时间线允许反向共存；同一时间线只允许一种方向但允许同方向追加；上级方向只影响结构属性和风险预算，不阻断下级交易。
...
- [x] 2026-07-01 回测与批量回测页面同步独立时间线口径：不再显示“4h 执行”旧框架，风险输入和结果表改为“风险预算 / 独立时间线”；回测配置快照新增 `trade_policy_version=INDEPENDENT_TIMELINES_V1`，批量回测参数 key/日志也带政策版本，避免继续复用独立时间线改造前的旧归档结果。
- [x] 2026-07-01 回测执行已补齐策略预热窗口：用户选择的 3个月 / 6个月 / 1年 / 2年只作为交易统计窗口，回测会额外向前拉取周线级别 warmup K 线预热 MA/ATR/DMI/BOLL，与实时 Paper 的 `warmup_klines` 机制一致；批量回测数据库同步、cache 和 checkpoint 同步记录 warmup 起点，旧策略政策归档不再进入当前结果表。验证 BTCUSDT 1年 EMA15/MA60：普通回测 82 笔，最小批量 smoke 90 笔。
