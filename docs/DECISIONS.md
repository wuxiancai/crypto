...
> 2026-06-30 更新：本决策已被 D15 对当前 Paper/Backtest/Web 策略主线取代。以下内容仅作为历史实现背景，不再作为运行策略内核。
...
> 2026-06-30 更新：本决策已被 D15 对当前 Paper/Backtest/Web 策略主线取代。新版 `WEEKLY_DAILY_H4_V1` 使用 `WEEKLY / DAILY / H4` 层级和日线反弹/顺势互斥规则，不再使用旧 hedge bucket 语义。
...
旧 `LAYERED_DAILY_V1` 仅作为历史实现背景存在；正式运行链路必须切换到新版内核。禁止把 `DAY_CORE / FOUR_HOUR_ADDON / FOUR_HOUR_HEDGE` 直接解释为新版 `WEEKLY / DAILY / H4`。

同一次 Paper 或 Backtest 运行只能启用一个策略内核。新版内核切为默认前，必须完成文档、单测、回测对照、状态页展示和 Handoff 更新；切为默认后，Paper/Backtest/Web 状态页必须百分百按 `WEEKLY_DAILY_H4_V1` 执行，不能再通过旧开关回落到旧内核。

### D16. trade_policy.md 是独立时间线策略政策源

2026-07-01 起，`trade_policy.md` 是当前 `WEEKLY_DAILY_H4_V1` 时间线交易政策的业务源文档。

当前策略只允许 `WEEKLY / DAILY / H4` 三种时间线单。三种时间线各自拥有独立策略，独立决定开仓、止盈、止损、减仓和退出。不同时间线之间不互相过滤、不互相阻断、不互相强制平仓。

同一交易对可以同时持有不同时间线的反向仓，例如周线多、日线空和 4H 多可以共存。同一交易对的同一时间线只能持有一种方向：已有同时间线多单时禁止再开空单，已有同时间线空单时禁止再开多单；同一时间线同方向允许追加仓位。

上级时间线方向只用于定义结构属性和风险预算，不用于阻断下级时间线交易。日线参考周线方向来区分主方向单、反弹单或中性单；4H 参考日线方向来区分主方向单、反弹单或中性单。顺上级方向表示结构风险较低、风险预算较高；逆上级方向表示结构风险较高、风险预算较低。
