...
- 2026-07-02 周线仓位过小修复：
  ...
  - 根因：`WEEKLY_DAILY_H4_V1` 周线仓已经在交易层跳过普通止损/固定止盈，只由周线策略层的阶段减仓和强制退出管理；但开仓 sizing 仍沿用普通“权益 * 风险预算 / 止损距离”。周线初始防线使用 MA60，距离价格很远，且该防线并不会作为普通止损撮合触发，导致周线仓被虚拟止损距离错误压小。
  - 修复：`PaperConfig` / `StrategyBacktestConfig` 新增 `weekly_margin_pct=0.10`。周线内核仓改用 `权益 * weekly_margin_pct * weekly_leverage / entry_price` 计算数量；日线和 4H 仍用原来的止损风险 sizing。周线保证金预算不再计入普通 planned-risk 汇总，但仍受总名义敞口、周线杠杆、强平保护和周线生命周期退出约束。
  - 页面和归档：普通回测页新增“周线保证金预算”；URL query、配置 payload、历史摘要均记录 `weekly_margin_pct`，避免以后只看到周线风险预算却看不到真实 sizing 依据。
  - 本地缓存 2 年 BTCUSDT 默认回测验证：总结果 `total_trades=340`、`net_pnl=227.96`、`final_equity=1227.96`；按层级：`WEEKLY=87.30`、`H4=151.41`、`DAILY=-10.74`。其中 `WEEKLY_LONG_TREND=76.08`、`WEEKLY_SHORT_TREND=11.22`。周线结果已从修复前 `1.79` 回到符合截图趋势幅度的数量级。
  - 验证：`.venv/bin/python -m pytest -q` 为 `250 passed`；关键文件 `py_compile` 和 `git diff --check` 通过。
