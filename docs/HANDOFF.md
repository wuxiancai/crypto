...
- 2026-06-24 Paper Runtime Web 复盘页：
  - 新增 `/paper/events` 只读页，复用 `paper_runtime_events`，支持按 `limit`、`event_type`、`symbol`、`strategy_type`、`bucket` 过滤，表格展示 signal / rejected_signal / fill / snapshot 摘要。
  - 模拟交易看板顶部新增“Paper复盘”入口，方便从运行页直接跳转。
  - 本机验证：`.venv/bin/python -m pytest tests/test_v1_2_paper_runtime_events_web.py tests/test_v1_0_paper_status_web.py -q`，19 passed。
...
3. 下一步继续真实行情 Paper Trading：增强 `/paper/events` 详情展开，例如展示完整 signal reason、condition_statuses、开仓参数、持仓快照 JSON，并增加按时间范围筛选。
