...
- 已实现真实行情 Paper runner 与脚本入口：`scripts/run_paper_realtime.py` 会连接 Binance WebSocket 已收盘 K 线流，并使用持久化 Paper stream runner 保存状态。当前默认策略函数为安全 `WAIT`，后续需要接入多周期策略编排后才会产生模拟交易。
...
3. 下一步继续真实行情 Paper Trading：把多周期策略编排接入 `run_paper_realtime.py`，或先实现运行报告和 Stop/Liquidation Guard 演练脚本。
