...
- 策略回测页默认使用 EMA50 / EMA200、历史 K 线 1500 根、初始资金 1000 USDT；页面允许用户输入 EMA 快线、EMA 慢线和历史 K 线根数，用于快速比较 EMA50/EMA200、EMA30/EMA120 等参数组合。1500 是 Binance 单次 REST 拉取的快速回测上限，不等同于 3-6 个月长期验证。
...
5. 下一步可把 `/backtest` 升级为分页历史回测：支持选择最近 3 个月 / 6 个月 / 1 年，自动分页拉取 Binance 历史 K 线，并输出最大回撤、胜率、盈亏比、按交易对统计和参数组合对比表。
