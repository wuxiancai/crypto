...
这些规则已经在 Paper/Backtest 的策略子仓撮合中复用；旧 `open_position` 只作为兼容字段保留。
...
- 当前权益
- 当前持仓
- 最近历史 fills（默认最多保留 1000 条，完整复盘看数据库事件表）
- rejected signals
- 最近策略评估
- 运行时间

项目支持从这个文件恢复 Paper 状态，避免重启后丢失上下文。这个 JSON 是运行态快照，不作为长期成交流水库；长期运行至少 1 年时，完整 signal / rejected_signal / fill / snapshot 复盘以 PostgreSQL 的 `paper_runtime_events` 为准。
